...
function renderPromptKindLine(event) {
  return event.promptKind ? `Kind: ${event.promptKind}` : null;
}
...
export function renderTranscript(meta, events) {
  ...
  const renderSingleEvent = (event) => {
    ...
    const promptKindLine = renderPromptKindLine(event);
    if (promptKindLine) {
      lines.push(promptKindLine, "");
    }
